...
class Controller implements ActionListener, MouseListener, KeyListener
{
	...
	public void mousePressed(MouseEvent e)
	{
		...
		if (e.getButton() == 3){
			...
			for(int i = 1;i < model.items.size();i++){
				if(distance(model.items.get(i), e) < distance(model.items.get(i-1), e)){
					closest = i;
				}
			}
			model.items.remove(closest);
			}
		}
	}
	...
	double distance(MapItem a, MouseEvent e){
		int x = a.x - e.getX() - this.view.scrollX;
		int y = a.y - e.getY() - this.view.scrollY;
